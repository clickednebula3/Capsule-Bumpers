...
if (_coll != noone) {
	if (!(global.plyrInputs[playerId][KEY_DOWN][0]) || !_coll.canThrough) {
		...
		if (!instance_exists(passthroughGround)) {
			passthroughGround = _coll;
		}
...
{ wantX = x - 100*global.plyrInputs[playerId][KEY_LEFT][0]; }
//move right
if (global.plyrInputs[playerId][KEY_RIGHT][0] && canMove)
{ wantX = x + 100*global.plyrInputs[playerId][KEY_RIGHT][0]; }
//jump
...
if (room == rm_jetpack_zone) { jetpackFuel = 10; }
